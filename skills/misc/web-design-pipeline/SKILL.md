...
> Build this with **Next.js**, use **Framer Motion** for animations wherever appropriate (when the framework is Next.js 15+ and framer-motion is already a dependency) — staggered scroll reveals, count-up numbers, parallax, moving logo strip — stunning but never cheesy. For projects that prefer GSAP or where the user explicitly requests it, use GSAP instead. Read `claude.md` and build it in one go.

**Animation library choice:** Prefer Framer Motion when the project is Next.js 15+ (it's a first-class dependency and avoids the GSAP + React reconciliation issues). Prefer GSAP for non-React sites, vanilla JS, or when the user asks for it specifically. Both can deliver awwwards-tier motion — pick the one that matches the stack.
...
## 6. Deploy
GitHub (private repo) → Vercel → framework preset **Next.js** → add a **custom domain** (avoid shipping `*.vercel.app` to clients). After deploy, notify the client: use `gog gmail send` to email the link from `seyed@weblyfe.nl`.
...
- **Em dashes in customer-facing copy** — banned globally. See brand-voice skill, Global Typography Rules. Use period, comma, colon, or hyphen instead.
...
- **Starting image generation without checking API balance first.** Always check FAL_KEY balance before launching a batch of 6+ generations. Exhausted credits mid-session waste time.

## Resources
- getdesign.md · durable.com · demos.greensock.com · 21st.dev · awwwards.com
- `references/image-sourcing.md` — tiered approach: AI gen → Unsplash → client photos
- `references/google-reviews.md` — live Google Maps reviews → slider on client sites
