...
- OSRM routing coverage is best for Europe and North America, but the public OSRM endpoint can fail with SSL handshake errors or time out. If a command is explicitly marked blocked after timeout, do not retry the same route command; switch to another source or use a route-page/map distance fallback.
- For live travel planning with fares/schedules, `maps` is only a supporting skill for geography/distance. Use a travel-planning workflow/source for carrier schedules and prices.
