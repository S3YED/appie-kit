...
- **Token from .env:** Python's `open().readlines()` may fail on the protected .env file. Use `grep` or `awk` in terminal instead. The terminal tool shows `***` in output but the actual value is correct.===ME:vercel-deployment
