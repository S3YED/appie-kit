...
- **Profile photo URLs**: Google's `profile_photo_url` returns Google CDN URLs. Set `unoptimized={true}` on Next.js `Image` to avoid domain allowlist issues.===ME:social-proof-widgets