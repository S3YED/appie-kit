...
- Fleet endpoint path: `/cognify/recall`, `/cognify/ingest` (nginx reverse proxy)
- Client-local endpoint path: `/recall`, `/ingest` (NO `/cognify/` prefix)
- Namespace convention: `workspace:<agent-name>` (e.g. `workspace:appie-1`, `workspace:zeus`).
- Tenant: `"fleet"` for fleet Cognify, bot-specific (e.g. `"zeus"`) for client instances.
