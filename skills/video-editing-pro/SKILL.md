...
If a sentence says the same thing as the previous sentence in different words, cut it. For example, if one sentence establishes a concept and the next merely restates it with different words, cut the restatement. But if a subsequent sentence adds genuinely NEW information (a surprising claim or proof), keep it even if adjacent to a cut.
...
Kept: establishes scope and gives a concrete proof item. Correct cut of filler ending.

**What v1 cut from clip-02 (sentences 3–5, 12.6s):**
- List enumeration restating the same idea — correct cut
- However: a sentence with NEW information (surprising proof) was also cut — that is a v1 error

**seg-03 (escalation, 9.1s):**
The escalation framing lands correctly but the audience lost proof items that would have built momentum. The energy plateau corresponds to this gap.

**seg-04 (demo, clip-03, 10.1s):**
Kept all sentences including low-information connectors that could be cut to tighten the segment.

**seg-05 (vision, clip-05, 8.1s):**
The emphasis pause is correctly preserved, but the segment cuts mid-sentence — the audience is left hanging on an incomplete thought.

**seg-06 (CTA, clip-04, 10.1s):**
Language switch is jarring without a bridging visual. A 940ms dead pause was left in — the clearest timing error in the v1 edit.

### Prescribed re-cuts

1. **Hook**: trim opening connector — saves time, sharpens entry
2. **Restore the cut proof sentence** — strongest visual proof item
3. **Cut low-information demo sentences** — remove filler content
4. **Complete the vision sentence** — close the hanging thought
5. **Cut the dead pause** — trim to 150ms

These changes would reduce total runtime and fix the most retention-damaging moments.

---

## Cross-skill references

- `video-editing`: production workflow, shot setup, normalization commands
- `thumbnails`: thumbnail selection criteria (strongest frame, not prettiest frame)
- `seo-checklist`: same "first impressions compound" principle applies to hook optimization

---

## Motion Graphics with Remotion

Codified 2026-05-03. Example brand: custom font, gold/teal/dark/light palette.
...
Chosen pattern: text SCALE-PUNCH + background color flash + word appears from below. Brand-aligned, no VFX software, pure CSS/spring.
